...
import DeleteSectionDialog from '../dialog/delete';
import EditSectionDialog from '../dialog/edit';
import DataTableColumnHeader from './data-table-column-header';

const SectionNameCell = ({ row }) => {
...
const GroupNameCell = ({ row }) => {
    const { group, division, department } = row.original;
    return (
        <div>
            <div className="text-left text-xs font-medium">{department === 'N/A' ? '' : department}</div>
            <div className="text-left text-xs text-slate-700">{division === 'N/A' ? '' : division}</div>
            <div className="text-left text-xs text-slate-500">{group}</div>
        </div>
    );
};
        ...
        <DropdownMenu open={dropdownOpen} onOpenChange={setDropdownOpen}>
            ...
            <DropdownMenuContent align="end">
                ...
                <div className="flex flex-col space-y-1">
                    ...
                    <DropdownMenuItem asChild>
                        <DeleteSectionDialog section={row.original} closeDropdown={() => setDropdownOpen(false)} />
                    </DropdownMenuItem>
                </div>
            </DropdownMenuContent>
        </DropdownMenu>
...
const TableColumns = (sort, handleSort) => [
    {
        accessorKey: 'name',
        header: () => <DataTableColumnHeader sort={sort} title={'Section Name'} handleSort={handleSort} columnName={'name'} />,
        cell: ({ row }) => <SectionNameCell row={row} />,
    },
    {
        accessorKey: 'group',
    ...
    ...(canAny(['section_management.edit', 'section_management.delete'])
        ? [
              {
                  accessorKey: 'actions',
