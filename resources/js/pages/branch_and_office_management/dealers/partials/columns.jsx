...
import DeleteDealerDialog from '../dialog/delete';
import EditDealerDialog from '../dialog/edit';
import DataTableColumnHeader from './data-table-column-header';

const DealerCodeCell = ({ row }) => {
    const dealer_code = row.getValue('dealer_code');
    return <div className="text-left font-medium">{dealer_code}</div>;
};
...
const ActionsCell = ({ row }) => {
    ...
    return (
        <DropdownMenu open={dropdownOpen} onOpenChange={setDropdownOpen}>
            {canAny(['dealer_management.edit', 'dealer_management.delete'])}
            ...
            <DropdownMenuContent align="end">
                ...
                <div className="flex flex-col space-y-1">
                    ...
                    <DropdownMenuItem asChild>
                        <EditDealerDialog dealer={row.original} />
                    </DropdownMenuItem>
                    <DropdownMenuItem asChild>
                        <DeleteDealerDialog dealer={row.original} closeDropdown={() => setDropdownOpen(false)} />
                    </DropdownMenuItem>
                </div>
            </DropdownMenuContent>
        </DropdownMenu>
...
const TableColumns = (sort, handleSort) => [
    {
        ...
        header: () => <DataTableColumnHeader sort={sort} title={'Dealer Code'} handleSort={handleSort} columnName={'dealer_code'} />,
        cell: ({ row }) => <DealerCodeCell row={row} />,
    },
    {
        accessorKey: 'name',
        header: () => <DataTableColumnHeader sort={sort} title={'Dealer Name'} handleSort={handleSort} columnName={'name'} />,
