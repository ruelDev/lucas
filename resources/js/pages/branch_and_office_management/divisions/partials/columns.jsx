...
import DeleteDivisionDialog from '../dialog/delete';
import EditDivisionDialog from '../dialog/edit';
import DataTableColumnHeader from './data-table-column-header';

const DivisionNameCell = ({ row }) => {
...
const GroupNameCell = ({ row }) => {
    ...
    return <div className="text-left text-xs font-medium">{group}</div>;
};
            ...
            <DropdownMenuContent align="end">
                ...
                <div className="flex flex-col space-y-1">
                    ...
                    <DropdownMenuItem asChild>
                        <EditDivisionDialog division={row.original} />
                    </DropdownMenuItem>
                    <DropdownMenuItem asChild>
                        <DeleteDivisionDialog division={row.original} closeDropdown={() => setDropdownOpen(false)} />
                    </DropdownMenuItem>
                </div>
            </DropdownMenuContent>
        </DropdownMenu>
...
const TableColumns = (sort, handleSort) => [
    {
        accessorKey: 'name',
        header: () => <DataTableColumnHeader sort={sort} title={'Division Name'} handleSort={handleSort} columnName={'name'} />,
        cell: ({ row }) => <DivisionNameCell row={row} />,
    },
    {
        accessorKey: 'group',
    ...
    ...(canAny(['division_management.edit', 'division_management.delete'])
        ? [
              {
                  accessorKey: 'actions',
