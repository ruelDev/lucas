...
export default function DeleteDivisionDialog({ division, closeDropdown }) {
    const { delete: destroy } = useForm();

    const ReactSwal = withReactContent(Swal);

    const handleDivisionDelete = (e) => {
        e.preventDefault();
        ...
        ReactSwal.fire({
            ...
            if (result.isConfirmed) {
                ...
                destroy(route('division-management.destroy', division.id), {
                    preserveState: true,
                    preserveScroll: true,
                    onSuccess: (response) => {
                        ReactSwal.fire({
                    ...
                    onError: (response) => {
                        if (response.delete_division) {
                            ReactSwal.fire({
                                title: 'Error',
                                text: response.delete_division,
                                icon: 'error',
                                confirmButtonText: 'Ok, got it!',
                                confirmButtonColor: '#1B4298',
                            });
    ...
    return (
        <>
            {can('division_management.delete') && (
                <button onClick={handleDivisionDelete} className="w-full">
