...
export default function ReceiptConverterLogs({ receiptConverterData }) {
    const rawData = receiptConverterData.converterGenerationLogs;
    ...
    return (
        <Card className="border-border/60 border shadow-sm transition-all hover:shadow-md">
            ...
            <CardHeader className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <CardTitle className="text-brand-primary">Receipt Conversion Trend</CardTitle>

                    <CardDescription className="text-brand-primary/70">Daily conversion activity</CardDescription>
                </div>
            ...
            <CardContent>
                <ChartContainer config={chartConfig} className="h-[280px] w-full">
                    <BarChart data={chartData} margin={{ left: 12, right: 12 }}>
                        ...
                        <Bar dataKey="count" fill="#F2561C" radius={[6, 6, 0, 0]} />
                    </BarChart>
                </ChartContainer>
            </CardContent>
        </Card>
