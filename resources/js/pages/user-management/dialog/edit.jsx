...
export default function EditUser({ user, roles, errors: serverErrors = {}, old = {} }) {
    const breadcrumbs = [
        {
            title: 'Users',
    ...
    const { data, setData, put, processing, errors } = useForm({
        fname: old.fname ?? user.fname ?? '',
        mname: old.mname ?? user.mname ?? '',
        lname: old.lname ?? user.lname ?? '',
        email: old.email ?? user.email ?? '',
        company: old.company ?? user.company ?? '',
        position: old.position ?? user.position ?? '',
        isBranchDealer: old.isBranchDealer ?? String(user.isBranchDealer ?? '1'),
        isAlternateUser: old.isAlternateUser ?? user.isAlternateUser,
        location: old.location ?? user.head_office_user?.location ?? 'n/a',
        branchDealerId: old.branchDealerId ?? user.branchDealerId ?? 'n/a',
        organization: old.organization ?? user.organization ?? 'n/a',
        status: old.status ?? user.status ?? 'active',
        remarks: old.remarks ?? user.remarks ?? '',
        role: old.role ?? user.roles[0]?.name ?? '',
        expiration_date: old.expiration_date ?? user.expiration_date ?? '',
    });

    const [cbRoleOpen, setCbRoleOpen] = useState(false);
    const [cbRoleValue, setCbRoleValue] = useState(user.roles[0]?.name);

    const [cbBranchOpen, setCbBranchOpen] = useState(false);
    const [cbBranchValue, setCbBranchValue] = useState(user.branchDealerId);

    const [cbDealerOpen, setCbDealerOpen] = useState(false);
    const [cbDealerValue, setCbDealerValue] = useState(user.branchDealerId);

    const [cbOrgOpen, setCbOrgOpen] = useState(false);
    const [cbOrgValue, setCbOrgValue] = useState(user.organization);

    const [open, setOpen] = useState(false);
    const [selectDate, setSelectDate] = useState(() => {
        try {
            return user.expiration_date && isValid(parseISO(user.expiration_date)) ? parseISO(user.expiration_date) : null;
        } catch {
            return null;
        }
    });
    ...
    const handleDateSelect = (date) => {
        setSelectDate(date);
        setData('expiration_date', date ? format(date, 'yyyy-MM-dd') : '');
    };
    ...
    const handleUserSubmit = (e) => {
        ...
        ReactSwal.fire({
            ...
            html: `<p style="font-size: 16px">Are you sure you want to update this user?</p>`,
            icon: 'question',
            iconColor: '#1B4298',
            ...
            if (result.isConfirmed) {
                ...
                const submitData = normalizeForm({
                    ...data,
                    isBranchDealer: parseInt(data.isBranchDealer, 10),
                    branchDealerId: data.branchDealerId ? parseInt(data.branchDealerId, 10) : '',
                    group: data.group ? parseInt(data.group, 10) : '',
                    division: data.division ? parseInt(data.division, 10) : null,
                    department: data.department ? parseInt(data.department, 10) : null,
                    section: data.section ? parseInt(data.section, 10) : null,
                });
                ...
                put(route('user-management.update', user.id), {
                    ...
                    onSuccess: (response) => {
                        ReactSwal.fire({
                            title: 'Success',
                            text: response.props.flash?.success || 'User updated successfully',
                            icon: 'success',
                            iconColor: '#1B4298',
                    ...
                    onError: (errors) => {
                        ReactSwal.fire({
                            title: 'Error',
                            text: errors.edit_user || 'Failed to edit User',
                            icon: 'error',
                            confirmButtonText: 'Ok, got it!',
                            confirmButtonColor: '#1B4298',
                        });
                                ...
                                <form onSubmit={handleUserSubmit} className="mx-3 mt-5">
                                    <div className="items mt-4 flex">
                                        <div className="mx-auto mt-2 w-full max-w-5xl">
                                            <div className="flex items-start justify-between gap-6">
                                                <div className="flex items-start gap-6">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-muted-foreground text-sm">Toggle to activate or deactivate this user</p>

                                                        <span
                                                            className={`mt-2 inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium ${
                                                                data.status === 'active' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600'
                                                            }`}
                                                        >
                                                ...
                                                <div className="max-w-sm flex-1">
                                                    {data.status !== 'active' && (
                                                        <div className="space-y-2">
                                                            ...
                                                            <Select
                                                                value={data.remarks ?? ''}
                                                                onValueChange={(value) => setData('remarks', value === 'none' ? null : value)}
                                                            >
                                            ...
                                            {errors.status && <p className="mt-3 text-sm text-red-500">{errors.status}</p>}
                                        </div>
                                    </div>
                                    <Separator className="mt-5" />
                                    <div className="mt-5">
                                        <div>
                                            <Label html="employee_id" className="mb-2">
                                        ...
                                        <div className="mt-5 grid grid-cols-1 gap-2 space-y-3 lg:grid-cols-3">
                                            <div>
                                                <Label html="lname" className="mb-2">
                                            ...
                                            <div>
                                                <Label html="fname" className="mb-2">
                                        ...
                                        <div className="mt-5">
                                            <Label html="email" className="mb-2">
                                    ...
                                    <div className="mt-5">
                                        <div className="grid grid-cols-1 gap-2 lg:grid-cols-2">
                                            <div>
                                                <Label html="position" className="mb-2">
                                            ...
                                            <div>
                                                <Label html="company" className="mb-2">
                                        ...
                                        {data.company !== 'DEALER' && (
                                            <div className="mt-5">
                                                ...
                                                <RadioGroup
                                                    defaultValue={String(data.isBranchDealer)}
                                        ...
                                        <div>
                                            {String(data.isBranchDealer) === '1' && data.company !== 'DEALER' && (
                                                <div className="mt-5">
                                                    <Label html="branchDealerId" className="mb-2">
                                            ...
                                            {data.company === 'DEALER' && (
                                                <div className="mt-5">
                                                    <Label html="branchDealerId" className="mb-2">
                                                        Dealer Branch <span className="text-red-500">*</span>
                                                    </Label>
                                            ...
                                            {String(data.isBranchDealer) === '2' && data.company !== 'DEALER' && (
                                                <div>
                                                    <div className="mt-5">
                                                    ...
                                                    <div className="grid grid-cols-1 gap-2 lg:grid-cols-1">
                                                        <div className="mt-5">
                                                            <Label html="organization" className="mb-2">
                                    ...
                                    <div className="mt-5">
                                        <div>
                                            <Label html="role_id" className="mb-2">
                                    ...
                                    <div>
                                        <span className="mt-3 text-xs text-neutral-600 italic">
                                            Note: In case that the creation of user is unsuccessfull, the possible reason is that it might be soft
