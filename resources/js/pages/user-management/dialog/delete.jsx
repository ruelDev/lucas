...
export default function DeleteUserDialog({ user, closeDropdown }) {
    ...
    const handleUserDelete = (e) => {
        ...
        ReactSwal.fire({
            ...
            <span>It's recommended to change the status instead, as the ID and email will remain in the system</span>
            </div>`,
            icon: 'warning',
            iconColor: '#1B4298',
            ...
            if (result.isConfirmed) {
                ...
                destroy(route('user-management.destroy', user.id), {
                    ...
                    onError: (response) => {
                        if (response.delete_user) {
                            ReactSwal.fire({
                                title: 'Error',
                                text: response.delete_user,
                                icon: 'error',
    ...
    return (
        <>
            {can('user_management.delete') && (
                <button onClick={handleUserDelete} className="w-full">
